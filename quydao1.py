...
syms m;
t = 1 / dm_dt; % dt/dm = 1 / dm_dt --> dt = ( 1/dm_dt ) * dm
t_het_nhien_lieu = double(int (t, m, m0, mt));
...
t_vector_roi = linspace(t_len_dinh, 1000, 1000); 
% dv/dt = -g ( a = -g ; chọn chiều dương hướng lên )
...
%vẽ đồ thị 2D
figure('Name', 'Đồ thị Vận tốc của Tên Lửa theo Thời gian 2D');
plot(t_full, v_full, 'LineWidth', 2);
...
figure('Name', 'Đồ thị Độ cao của Tên Lửa theo Thời gian 2D');
plot(t_full, h_full , 'LineWidth', 2);
